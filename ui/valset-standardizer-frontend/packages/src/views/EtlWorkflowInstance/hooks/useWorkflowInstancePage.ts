...
import {
  ...
  getEtlWorkflowInstanceStateCount,
  ...
  type WorkflowInstanceStateCountDTO,
  type WorkflowInstanceStateCountPageDTO,
  type WorkflowInstanceStateCountQueryParams,
...
const STATUS_CARD_ORDER: WorkflowInstanceStatus[] = [
  "SUBMITTED",
  "RUNNING",
  "SUCCEEDED",
  "FAILED",
...
const STATUS_CARD_TONE: Record<WorkflowInstanceStatus, WorkflowInstanceStateStatRow["tone"]> = {
  DRAFT: "default",
  READY: "default",
...
const formatTaskTypeLabel = (value?: string | null) => {
  ...
  return taskType.toUpperCase() === "SUB_WORKFLOW" ? "阶段任务" : "任务";
};
...
const mapTaskRow = (task: WorkflowTaskInstanceDTO): WorkflowTaskRow => {
  ...
  return {
    ...
    taskType: formatTaskTypeLabel(task.taskType),
...
export const useWorkflowInstancePage = (): { page: WorkflowInstancePage } => {
  ...
  const workflowStateLoading = ref(false);
  const workflowStateCards = ref<WorkflowInstanceStateStatRow[]>([]);
  ...
  const definitionMap = computed(() => {
    ...
    definitions.value.forEach((definition) => {
      if (
        ...
        definition.workflowVersionNo !== undefined
      ) {
        map.set(
  ...
  const succeededCount = computed(
    () =>
      rows.value.filter((row) => normalizeStatus(row.status) === "SUCCEEDED")
        .length,
  );
  ...
  const selectedWorkflowDescription = computed(() => {
    ...
    if (!definition) {
      return "请选择一个 DolphinScheduler 工作流定义后查看对应的状态统计和实例列表。";
    }
    const stageCount = (definition.stages ?? []).length;
    return `当前定义包含 ${stageCount} 个阶段，状态统计与列表都按该定义对应的项目编码查询。`;
  });

  const mapStateCards = (
    totalCount: number,
    counts: WorkflowInstanceStateCountDTO[],
  ): WorkflowInstanceStateStatRow[] => {
    const countMap = new Map<string, number>();
    counts.forEach((item) => {
      const status = normalizeStatus(item.state);
      countMap.set(status, Number(item.count ?? 0));
    });
    return [
      {
        statKey: "TOTAL",
        label: "工作流实例总数",
        value: Number(totalCount ?? 0),
        desc: "当前工作流定义与时间范围内的工作流实例总量",
        tone: "primary",
      },
      ...STATUS_CARD_ORDER.map((status) => ({
        ...
        desc: `状态 ${STATUS_LABELS[status] || status} 的工作流实例数量`,
  ...
  const applyDefaultWorkflowSelection = () => {
    ...
    const availableWorkflowCodes = new Set(
      workflowOptions.value.map((item) => item.value),
    );
    if (
      currentWorkflowCode &&
      availableWorkflowCodes.has(currentWorkflowCode)
    ) {
      return;
    }
  ...
  const currentFilterSummary = computed(() => {
    const segments = [
      ...
      query.workflowName && `名称=${query.workflowName}`,
      query.platformType && `平台=${formatPlatformLabel(query.platformType)}`,
      ...
      query.triggerTimeFrom &&
        `开始时间=${formatDateTime(query.triggerTimeFrom)}`,
      query.triggerTimeTo && `结束时间=${formatDateTime(query.triggerTimeTo)}`,
    ].filter(Boolean);
  ...
  const mapListRow = (item: WorkflowInstanceViewDTO): WorkflowInstanceRow => {
    ...
    const workflowName = formatText(
      item.workflowName || resolveWorkflowName(workflowCode, workflowVersionNo),
    );
    return {
  ...
  const hydrateTaskRows = async (
    targetRows: WorkflowInstanceRow[],
    ...
    await Promise.all(
      targetRows.map(async (row) => {
        ...
        try {
          ...
          const taskRows = (unwrapSingleResult(res)?.taskList ?? []).map(
            mapTaskRow,
          );
  ...
  const refreshExpandedTaskRows = async (requestId = listRequestId) => {
    const targetRows = rows.value.filter((row) =>
      expandedRowKeys.value.includes(row.instanceId),
    );
    await hydrateTaskRows(targetRows, requestId);
  };
  ...
  const loadList = async ({ silent = false }: { silent?: boolean } = {}) => {
    ...
    try {
      ...
      const params: WorkflowInstanceQueryParams = {
        ...
        platformType: (query.platformType || undefined) as
          | EtlPlatformType
          | undefined,
      ...
      pagination.value.current = Number(res.pageIndex ?? pageIndex);
      pagination.value.pageSize = Number(res.pageSize ?? pageSize);
      syncSelectedRow();
      await refreshExpandedTaskRows(requestId);
    } catch (error) {
      if (requestId !== listRequestId) {
        return;
      }
      message.error("ETL 工作流实例列表失败");
      throw error;
    } finally {
  ...
  const loadWorkflowStateStats = async (
    { silent = false }: { silent?: boolean } = {},
  ) => {
    const definition = selectedDefinition.value;
    const projectCode = selectedProjectCode.value;
    if (!definition || !projectCode) {
      workflowStateCards.value = [];
      return;
    }
    if (!silent) {
      workflowStateLoading.value = true;
    }
    try {
      ...
      workflowStateCards.value = mapStateCards(
        Number(page.totalCount ?? 0),
        page.workflowInstanceStatusCounts ?? [],
      );
    } catch {
      workflowStateCards.value = [];
      message.error("加载工作流实例状态统计失败");
    } finally {
      if (!silent) {
        workflowStateLoading.value = false;
      }
    }
  };

  const refreshListAndStats = async () => {
    await Promise.allSettled([loadList(), loadWorkflowStateStats()]);
  };
  ...
  const handleWorkflowCodeChange = (workflowCode: unknown) => {
    ...
    query.workflowVersionNo =
      ...
      latestDefinition.workflowVersionNo !== null
        ...
        : "";
    query.workflowName = String(latestDefinition?.workflowName ?? "").trim();
    query.stageCode = "";
  };
  ...
  const openTaskInstancePage = (
    ...
    router.push({
      ...
      query: {
        ...
        workflowVersionNo: String(row.workflowVersionNo),
        workflowInstanceName: taskRow.workflowInstanceName || undefined,
      },
    });
  };
  ...
  const refreshAfterMutation = async (
    response: WorkflowInstanceDTO | undefined,
  ) => {
    if (response) {
  ...
  const rerunFailedTasks = async (row: WorkflowInstanceRow) =>
    submitTrigger(row, "START_FAILURE_TASK_PROCESS", "已提交失败任务重跑");
  ...
  const resumeInstance = async (row: WorkflowInstanceRow) => {
    actionLoading.value = true;
    try {
      const res = await resumeEtlWorkflowInstance(row.instanceId, {
        reason: "恢复运行",
      });
      const next = unwrapSingleResult(res);
      await refreshAfterMutation(next);
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      await Promise.all([
        ...
        loadWorkflowStateStats({ silent: true }),
      ]);
    } catch (error) {
      message.error("初始化 工作流实例页失败");
      throw error;
    } finally {
      loading.value = false;
    }
  ...
  const formatPlatformLabel = (value?: EtlPlatformType | string | null) => {
    ...
    return (
      metadata?.platformName || PLATFORM_LABELS[platformType] || platformType
    );
  };
  ...
  const canRerunInstance = (row: WorkflowInstanceRow) =>
    isSucceededStatus(row.status) || normalizeStatus(row.status) === "STOPPED";
  ...
  const canResumeInstance = (row: WorkflowInstanceRow) =>
    normalizeStatus(row.status) === "STOPPED" &&
    row.platformType === "DOLPHIN_SCHEDULER";
