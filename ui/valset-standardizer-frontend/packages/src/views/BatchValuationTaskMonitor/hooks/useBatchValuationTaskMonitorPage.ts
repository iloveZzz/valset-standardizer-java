...
import {
  executeBatchValuationTask,
  pageBatchValuationTasks,
  retryBatchValuationTask,
  retryBatchValuationTaskStep,
  stopBatchValuationTask,
  type BatchValuationTaskActionResultDTO,
...
import {
  getBatchValuationTaskTrace,
  type BatchValuationTaskTraceDTO,
  type BatchValuationTaskTraceRecordDTO,
} from "@/api/batchValuationTaskTrace";
...
import type {
  BatchValuationTaskMonitorPageState,
  ValuationTraceEdge,
  ValuationTraceNode,
...
const defaultQuery = (): BatchValuationTaskQueryState => ({
  batchId: "",
...
const normalizeStatus = (value?: string): BatchValuationTaskStatus | "MISSING" => {
  ...
  if (
    ...
    return status as BatchValuationTaskStatus | "MISSING";
  }
...
const statusLabel = (value?: string) => {
  ...
  return batchValuationTaskStatusCatalog.find((item) => item.status === status)?.label ?? status;
};
...
const toBatchRow = (row: BatchValuationTaskBatchDTO): BatchValuationTaskBatchRow => ({
  ...row,
...
const toStepRow = (step: BatchValuationTaskStepDTO): BatchValuationTaskStepRow => ({
  ...step,
...
const recordPayload = (record?: BatchValuationTaskTraceRecordDTO) => ({
  ...record,
...
const recordNodeData = (
  ...
  record?: BatchValuationTaskTraceRecordDTO,
): ValuationTraceNodeData =>
  createNodeData({
    kind,
...
const buildGraph = (
  ...
  fallbackBatch: BatchValuationTaskBatchRow | null,
  ...
  const parseTaskRecord: BatchValuationTaskTraceRecordDTO = {
  ...
  const resultRecord: BatchValuationTaskTraceRecordDTO = {
...
export const useBatchValuationTaskMonitorPage = (): {
  page: BatchValuationTaskMonitorPageState;
} => {
  ...
  const selectedTrace = ref<BatchValuationTaskTraceDTO | null>(null);
  const nodes = ref<ValuationTraceNode[]>([]);
  const edges = ref<ValuationTraceEdge[]>([]);
  const detailVisible = ref(false);
  const selectedDetail = ref(null as BatchValuationTaskMonitorPageState["selectedDetail"]);
  let listRequestId = 0;
  ...
  const loadList = async (options: { append?: boolean } = {}) => {
    ...
    try {
      ...
      const params: BatchValuationTaskQueryParams = {
        ...query,
        pageIndex,
        pageSize: PAGE_SIZE,
      };
      const res = await pageBatchValuationTasks(params);
  ...
  const loadTrace = async (batchId: string, options: { silent?: boolean } = {}) => {
    ...
    try {
      const res = await getBatchValuationTaskTrace(batchId);
  ...
  const selectTask = async (row: BatchValuationTaskBatchRow) => {
  ...
  const runAction = async (
    runner: (batchId: string) => Promise<{ data?: BatchValuationTaskActionResultDTO }>,
    successText: string,
  ) => {
    if (!selectedBatchId.value) {
      message.warning("请先选择任务");
  ...
  const executeSelected = () =>
    runAction((batchId) => executeBatchValuationTask(batchId, { reason: "链路监控手动执行" }), "已提交执行");

  const retrySelected = () =>
    runAction((batchId) => retryBatchValuationTask(batchId, { reason: "链路监控全流程重跑" }), "已提交全流程重跑");

  const stopSelected = () =>
    runAction((batchId) => stopBatchValuationTask(batchId, { reason: "链路监控停止" }), "已提交停止");

  const retryStep = async (step: BatchValuationTaskStepRow) => {
    ...
    actionLoading.value = true;
    try {
      const res = await retryBatchValuationTaskStep(selectedBatchId.value, step.stepId, {
        reason: "链路监控步骤重跑",
      });
  ...
  const canRetryStep = (step?: BatchValuationTaskStepRow | null) =>
  ...
  const getTaskInitial = (row?: BatchValuationTaskBatchRow | null) => {
    const text =
      row?.productName ||
      row?.productCode ||
      row?.managerName ||
  ...
  return {
    page: reactive({
      ...
      getTaskInitial,
      formatJson,
    }) as unknown as BatchValuationTaskMonitorPageState,
  };
};
