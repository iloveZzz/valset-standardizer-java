...
const workflowColumns = computed<YTableColumn[]>(() => [
  ...
  { field: "bindingSummary", title: "绑定摘要", minWidth: 220, ellipsis: true },
  {
    field: "action",
    title: "操作",
...
const pageTitle = computed(() =>
  isDefinitionMode.value ? "工作流列表与执行平台绑定" : "工作流列表与阶段定义",
);

const pageDescription = computed(() =>
  isDefinitionMode.value
    ? "点击“编辑”打开工作流表单，保存会写入当前工作流定义。"
    : "点击“新增阶段”打开阶段定义窗体，保存会把阶段追加到当前工作流版本。",
);
...
const stageModalTitle = computed(() =>
  isStageMode.value ? `阶段定义 - ${workflowLabel.value}` : "阶段定义",
);
...
const isWorkflowOnline = (row: (typeof tableData.value)[number]) => {
  const releaseState = String(row.externalReleaseState ?? "").trim().toUpperCase();
  ...
  <div class="etl-workflow-page">
    <YCard class="etl-workflow-table-card" :bordered="false" :padding="12">
      ...
      >
        ...
        <template #action="{ row }">
          <template v-if="isDefinitionMode">
            <YButton
              size="small"
              type="link"
              @click="openEditWorkflowDialog(row)"
            >
              编辑
            </YButton>
            <YButton
              v-if="row.supportedOperations?.includes('SYNC')"
              size="small"
              type="link"
              @click="confirmSyncWorkflow(row)"
            >
              同步工作流
            </YButton>
            <YButton
              v-if="row.supportedOperations?.includes('ONLINE')"
              size="small"
              type="link"
              :disabled="row.syncStatus !== 'SYNCED' || isWorkflowOnline(row)"
              @click="confirmOnlineWorkflow(row)"
            >
              上线
            </YButton>
            <YButton
              v-if="row.supportedOperations?.includes('OFFLINE')"
              size="small"
              type="link"
              :disabled="row.syncStatus !== 'SYNCED' || isWorkflowOffline(row)"
              @click="confirmOfflineWorkflow(row)"
            >
              下线
            </YButton>
            <YButton
