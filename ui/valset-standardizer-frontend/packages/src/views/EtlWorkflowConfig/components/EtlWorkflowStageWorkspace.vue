...
import type { EtlWorkflowConfigPage } from "../types";
...
const createStageDraft = (stageOrder = 1) => ({
  stageCode: "",
  stageName: "",
  stageOrder,
  description: "",
...
const stageDraft = reactive(createStageDraft(1));
...
const pageTitle = computed(() => "工作流列表与阶段定义");
const pageDescription = computed(
  () => "按工作流分组展示阶段定义，展开分组后可以查看阶段明细并新增阶段。",
);
...
const openStageDialog = async (
  ...
  Object.assign(
    stageDraft,
    createStageDraft((page.formState.stages?.length ?? 0) + 1),
  );
...
const expandStageRows = (row: (typeof workflowRows.value)[number]) =>
  ...
  <div class="etl-workflow-page">
    <YCard class="etl-workflow-table-card" :bordered="false" :padding="12">
      ...
      >
        ...
        <template #action="{ row }">
          <YButton size="small" type="link" @click="openStageDialog(row)">
            新增阶段
          </YButton>
        </template>
        ...
        <template #expand-row="{ row }">
          <div class="etl-workflow-stage-expand">
            <div class="etl-workflow-stage-expand__header">
              <strong>{{ row.workflowCode }} · {{ row.workflowName }}</strong>
              <span
                >{{ row.workflowVersionNo }} · {{ row.bindingSummary }}</span
              >
            </div>
            <YTable
              class="etl-workflow-stage-table"
              :columns="stageColumns"
              :data="expandStageRows(row)"
              :row-dragable="true"
    ...
    <a-modal
      ...
      :title="`新增阶段 - ${page.formState.workflowCode || '未选择'}`"
      :width="'50vw'"
      :style="{ top: '10vh' }"
