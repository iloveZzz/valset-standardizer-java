...
新增“估值表解析任务管理”页面，用于串联估值表核心模块的数据处理链路，后端执行主链路已切到 批量任务，不纳入 transfer 分拣投递模块。

页面覆盖从估值表文件解析、结构标准化、估值贴源数据落地，到后续数据加工任务的全链路任务管理能力，重点解决以下问题：
...
| `STANDARD_LANDING`      | 估值贴源数据落地 | STG/DWD/标准持仓/估值数据写入      | 落地任务                                |
