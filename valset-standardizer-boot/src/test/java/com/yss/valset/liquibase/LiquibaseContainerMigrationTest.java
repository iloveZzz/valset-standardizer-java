...
class LiquibaseContainerMigrationTest {
    ...
    void migratesMysqlSchema() throws Exception {
        verifyMigration(
                ...
                MYSQL.getUsername(),
                MYSQL.getPassword(),
                "select count(*) from information_schema.tables where table_schema = database() and table_name in ('leaf_alloc','t_valset_workflow_task','t_ods_valuation_sheet_style','t_stg_external_valuation','t_dwd_external_valuation_subject','t_dwd_external_valuation_metric','t_ods_standard_subject','t_transfer_object','t_transfer_mail_info','t_transfer_rule','t_transfer_route','t_transfer_delivery_record','t_transfer_target','t_transfer_tag','t_transfer_object_tag','scheduled_tasks')",
    ...
    private void verifyMigration(String jdbcUrl, String username, String password, String tableCountSql, String indexCountSql, String realStorageColumnSql, String mailInfoColumnSql, String sourceCountSql, String targetCountSql, String ruleCountSql, String routeCountSql) throws Exception {
        ...
        try (Connection verifyConnection = DriverManager.getConnection(jdbcUrl, username, password)) {
            assertThat(scalarCount(verifyConnection, tableCountSql)).isEqualTo(16);
