package com.yss.subjectmatch.application.support;
...
public class TaskReuseService {
    ...
    public TaskReuseService(TaskGateway taskGateway) {
     ...
     */
    public TaskInfo findReusableSuccessfulTask(TaskType taskType, String businessKey, boolean forceRebuild) {
        if (forceRebuild || !supportsReuse(taskType)) {
            return null;
        }
        TaskInfo reusableTask = taskGateway.findLatestSuccessfulTask(taskType, businessKey);
