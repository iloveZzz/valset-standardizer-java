...
public class TrSpvJjhzgzbGatewayImpl implements TrSpvJjhzgzbGateway {

    private final TrSpvJjhzgzbRepository repository;
    ...
    public void saveStandardizedJjhzgzb(Long taskId, Long fileId, String sourceTp, String sourceSign, ParsedValuationData standardizedValuationData) {
        ...
        List<TrSpvJjhzgzbPO> rows = JjhzgzbStandardizationSupport.buildRows(standardizedValuationData, sourceTp, sourceSign, productBusinessFields);
        if (rows.isEmpty()) {
        ...
        insertRows(rows);
        TrSpvJjhzgzbPO firstRow = rows.get(0);
    ...
    private Set<SnapshotKey> validateRows(Long taskId, Long fileId, List<TrSpvJjhzgzbPO> rows) {
        Set<SnapshotKey> snapshotKeys = new LinkedHashSet<>();
        Set<String> uniqueKeys = new LinkedHashSet<>();
        int rowNumber = 0;
        for (TrSpvJjhzgzbPO row : rows) {
            rowNumber++;
    ...
    private void deleteExistingRows(Set<SnapshotKey> snapshotKeys) {
        for (SnapshotKey snapshotKey : snapshotKeys) {
            repository.delete(Wrappers.lambdaQuery(TrSpvJjhzgzbPO.class)
                    .eq(TrSpvJjhzgzbPO::getPdCd, snapshotKey.pdCd)
                    .eq(TrSpvJjhzgzbPO::getOrgCd, snapshotKey.orgCd)
                    .eq(TrSpvJjhzgzbPO::getBizDate, snapshotKey.bizDate));
        }
    }

    private void insertRows(List<TrSpvJjhzgzbPO> rows) {
