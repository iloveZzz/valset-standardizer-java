...
public class DolphinSchedulerWorkflowSyncSupport {
    ...
    private static final int MAX_DELETE_RETRY_TIMES = 3;
    private static final long DELETE_RETRY_BACKOFF_MILLIS = 300L;

    @Nullable
    private final DolphinSchedulerRemoteApi remoteApi;
    private final DolphinSchedulerResponseSupport responseSupport;

    public DolphinSchedulerWorkflowSyncSupport(@Nullable DolphinSchedulerRemoteApi remoteApi,
    ...
    private long createProject(WorkflowDefinitionDTO definition, String projectName) {
        ...
        params.add("description", StringUtils.hasText(definition.getDescription())
                ...
                : projectName);
        JsonNode response = responseSupport.toJsonNode(remoteApi.createProject(params));
    ...
    private JsonNode syncWorkflow(long projectCode,
        ...
        params.add("locations", responseSupport.getObjectMapper().valueToTree(locations).toString());
        params.add("timeout", "0");
        params.add("taskRelationJson", responseSupport.getObjectMapper().valueToTree(taskRelations).toString());
        params.add("taskDefinitionJson", responseSupport.getObjectMapper().valueToTree(taskDefinitions).toString());
        params.add("executionType", "PARALLEL");
        params.add("releaseState", "OFFLINE");
        JsonNode response;
        if (workflowCode == null) {
            response = responseSupport.toJsonNode(remoteApi.createWorkflowDefinition(projectCode, params));
        } else {
            response = responseSupport.toJsonNode(remoteApi.updateWorkflowDefinition(projectCode, workflowCode, params));
        }
        if (!isSuccess(response)) {
    ...
    private boolean workflowDefinitionExists(long projectCode, long workflowCode) {
        try {
            JsonNode response = responseSupport.toJsonNode(remoteApi.getWorkflowDefinition(projectCode, workflowCode));
    ...
    private void deleteWorkflowDefinition(long projectCode, long workflowCode) {
        ...
        for (int attempt = 1; attempt <= MAX_DELETE_RETRY_TIMES; attempt++) {
            JsonNode response = responseSupport.toJsonNode(remoteApi.deleteWorkflowDefinition(projectCode, workflowCode));
            if (isSuccess(response)) {
                return;
            }
            lastMessage = extractMessage(response);
            if (StringUtils.hasText(lastMessage) && lastMessage.contains("not exist")) {
                return;
            }
            if (!isTransientDeleteFailure(lastMessage) || attempt == MAX_DELETE_RETRY_TIMES) {
                throw new IllegalStateException("DolphinScheduler 删除工作流失败：" + lastMessage);
            }
    ...
    private void onlineWorkflowDefinition(long projectCode, long workflowCode, String name) {
        JsonNode response = responseSupport.toJsonNode(remoteApi.releaseWorkflowDefinition(projectCode, workflowCode, buildReleaseParams(name, "ONLINE")));
        if (!isSuccess(response)) {
    ...
    private void offlineWorkflowDefinition(long projectCode, long workflowCode, String name) {
        JsonNode response = responseSupport.toJsonNode(remoteApi.releaseWorkflowDefinition(projectCode, workflowCode, buildReleaseParams(name, "OFFLINE")));
        if (!isSuccess(response)) {
    ...
    private Long verifyProjectCode(long projectCode) {
        JsonNode response = responseSupport.toJsonNode(remoteApi.getProject(projectCode));
        if (!isSuccess(response)) {
            return null;
        }
    ...
    private Long findProjectCodeByName(String projectName) {
        ...
        JsonNode response = responseSupport.toJsonNode(remoteApi.listProjects(normalizeQueryParams(params)));
        if (!isSuccess(response)) {
            return null;
        }
    ...
    private boolean isTransientDeleteFailure(String message) {
        if (!StringUtils.hasText(message)) {
            return false;
        }
        return message.contains("Lock wait timeout exceeded")
    ...
    private void sleepBeforeRetry(int attempt) {
        try {
            Thread.sleep(DELETE_RETRY_BACKOFF_MILLIS * attempt);
        } catch (InterruptedException exception) {
            Thread.currentThread().interrupt();
            throw new IllegalStateException("DolphinScheduler 删除工作流重试被中断", exception);
        }
    }
