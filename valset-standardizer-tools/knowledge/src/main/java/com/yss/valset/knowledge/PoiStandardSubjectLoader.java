...
public class PoiStandardSubjectLoader implements StandardSubjectLoader {
    ...
    public List<StandardSubject> load(DataSourceConfig config) {
        ...
        try {
            if (SpreadsheetXmlSupport.isSpreadsheetXml(standardWorkbookPath)) {
                List<StandardSubject> subjects = loadSpreadsheetXml(standardWorkbookPath);
                log.info("SpreadsheetML 标准科目加载完成，workbookPath={}, count={}", standardWorkbookPath, subjects.size());
                return subjects;
            }
    ...
    private List<StandardSubject> loadSpreadsheetXml(Path standardWorkbookPath) throws IOException {
        List<List<String>> rows = SpreadsheetXmlSupport.readFirstSheetRows(standardWorkbookPath);
        List<BaseRow> baseRows = new ArrayList<>();
        Set<String> existingCodes = new LinkedHashSet<>();
