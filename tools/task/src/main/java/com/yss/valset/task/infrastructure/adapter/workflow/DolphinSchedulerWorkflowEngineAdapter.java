...
/**
 * DOLPHIN_SCHEDULER 工作流执行平台适配器。
 */
@Component
public class DolphinSchedulerWorkflowEngineAdapter implements WorkflowEngineAdapter {

    @Override
    public WorkflowEngineType engineType() {
        return WorkflowEngineType.DOLPHIN_SCHEDULER;
    }
    ...
    public void trigger(String workflowCode, String stageCode, Map<String, Object> context) {
        throw new UnsupportedOperationException("DOLPHIN_SCHEDULER 平台暂未接入执行能力");
    }
    ...
    private void validateType(WorkflowExecutorBindingDTO binding) {
        ...
        if (!WorkflowEngineType.DOLPHIN_SCHEDULER.name().equalsIgnoreCase(binding.getEngineType())) {
